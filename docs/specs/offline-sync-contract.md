...
| 服务端「定期消费」规则（房租等） | ❌ 排除 | 需要新表 + 一个 cron 去生成实例，而账户级 cron 配额已用 4/5、**仅余 1 个空位**（#15 已决议不做自动备份、不新增 cron ⇒ 该空位仍空置，但任何新增仍须先回 #6 重排）；用「模板」替代 —— 一键预填、仍需手动确认 |
...
> ⚠️ **本节 3 条派生要求已由 #15 全部落定**（`docs/specs/backup-export-contract.md` §4 / §10）：
> 设备侧数据（IndexedDB 队列与本地模板）已明确**排除在导出范围外**并写进契约的隐私约束；
> `entry.client_ref` 未被当作备份必需项。本节保留原文供追溯，**不再代表未决事项**。
